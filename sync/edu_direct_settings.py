# -*- coding: utf-8 -*-
"""
sync/edu_direct_settings.py — синк настроек кампаний Директа (все логины EDU) → edu_campaign_settings.
...
Запуск:  python -m sync.edu_direct_settings

ENV:
    DATABASE_URL
    DIRECT_TOKEN            (тот же токен, что sync/direct.py)
    DIRECT_CLIENTS_JSON     (список логинов vse/vuz/provuz/brand) или DIRECT_CLIENT_LOGIN
"""
...
_CURRENT_LOGIN = ""  # выставляется в цикле по логинам


def _token() -> str:
    return os.environ["DIRECT_TOKEN"]  # тот же токен, что sync/direct.py


def _client_login() -> str:
    if not _CURRENT_LOGIN:
        raise RuntimeError("_CURRENT_LOGIN не выставлен")
...
def _backfill_missing_crr(out: Dict[str, Dict[str, Any]], campaign_ids: List[str]) -> None:
    ...
    print(f"  [edu_direct_settings] CRR backfill для {len(missing)} кампаний (v501 campaigns.get)")
    ...
    for part in _chunked(missing, 100):
        for url in (CAMPAIGNS_V501_URL, CAMPAIGNS_URL):
            ...
            try:
                ...
                print(f"  [edu_direct_settings] CRR backfill {url}: {e}")
                continue
                    ...
                    for raw in (
                        ...
                        if isinstance(raw, dict):
                            ...
                            if isinstance(ac, dict) and not ac.get("Crr"):
                                print(
                                    f"  [edu_direct_settings] AverageCrr без Crr, cid={cid}, keys={list(ac.keys())}"
                                )
                                break
        ...
        for ch_key in ("network", "search"):
            ...
            if isinstance(ch, dict) and "CRR" in str(ch.get("biddingStrategyType") or "").upper():
                if not ch.get("targetDrr"):
                    ...
                    break
    if still_missing:
        print(f"  [edu_direct_settings] WARN: Crr не получен для {still_missing} CRR-кампаний")
...
def _fetch_campaigns_for_settings(campaign_ids: List[str]) -> Dict[str, Dict[str, Any]]:
    ...
    for part in _chunked(campaign_ids, 1000):
        ...
        if unified_ids:
            ...
            try:
                ...
                print(f"  [edu_direct_settings] v501 campaigns.get: {e}")
    ...
    if missing:
        print(f"  [edu_direct_settings] WARN: campaigns.get не вернул {len(missing)} кампаний, добираю")
        ...
        for part in _chunked(missing, 100):
            try:
                ...
                print(f"  [edu_direct_settings] campaigns.get retry батч: {e}")
        ...
        for cid in [c for c in missing if c not in out]:
            try:
                ...
                print(f"  [edu_direct_settings] campaigns.get id={cid} полные поля: {e}")
            if cid in out:
                continue
            try:
                ...
                for c in r.get("Campaigns") or []:
                    _parse_campaign(c)
                    print(f"  [edu_direct_settings] id={cid} добран только общими полями, Type={c.get('Type')}")
            except RuntimeError as e:
                print(f"  [edu_direct_settings] campaigns.get id={cid} общие поля: {e}")
...
def _fetch_autotargeting_by_campaign(
    ...
    for part in _chunked(campaign_ids, 5):
        ...
        try:
            ...
            print(f"  [edu_direct_settings] keywords/autotargeting skip: {e}")
...
def _fetch_geo_region_names(region_ids: List[int]) -> Dict[int, str]:
    ...
    for part in _chunked([str(x) for x in abs_ids], 500):
        ...
        try:
            ...
            print(f"  [edu_direct_settings] getGeoRegions skip: {e}")
            break
    out: Dict[int, str] = {}
...
def _fetch_smart_targets(campaign_ids: List[str]) -> Dict[str, Dict[str, Any]]:
    ...
    for part in _chunked(campaign_ids, 2):
        try:
            ...
            print(f"  [edu_direct_settings] smartadtargets полные поля: {e}; откат на офферный флаг")
            try:
                rows = list(_paginate_items(SMARTADTARGETS_URL, "SmartAdTargets", _body(minimal, part)))
            except RuntimeError as e2:
                print(f"  [edu_direct_settings] smartadtargets skip: {e2}")
                break
        for row in rows:
...
def _list_campaigns_for_login() -> Dict[str, str]:
    """{campaign_id: campaign_name} для текущего логина (не-архивные)."""
    body = {
        "method": "get",
        "params": {
    ...
    r = requests.post(CAMPAIGNS_URL, headers=_json_headers(),
                      data=json.dumps(body, ensure_ascii=False).encode("utf-8"), timeout=120)
    r.raise_for_status()
    items = (r.json().get("result") or {}).get("Campaigns") or []
...
def sync_edu_campaign_settings() -> int:
    ...
    for client in _direct_clients():
        ...
        try:
            names = _list_campaigns_for_login()
            campaign_ids = sorted(names.keys())
            if not campaign_ids:
                print(f"[edu_direct_settings] {login}: 0 кампаний")
                continue
            n = _sync_campaign_settings(campaign_ids, names)
