...
def collect_ideas(
    *,
    ...
    # доставали лишь то, что генератор находит сегодня заново. Строка, которую
    # он перестал находить, оставалась в очереди навсегда с числами дня
    # заведения — 29.08.2026 так висели две идеи proven с ожиданием 32 ₽ при
    # цене замера 933 ₽ и 129 ₽ при 829 ₽. Недоступный реестр прогон не роняет
    # по тому же правилу, что и остальные его чтения: расчётный такт считает
    # деньги, и падать из-за экрана предложений ему нельзя.
    swept: List[Dict[str, Any]] = []
    sweep_error = None
    try:
        swept = ideas_registry.sweep_open()
    except Exception as exc:  # noqa: BLE001
    ...
    return {
        ...
        # Что проход закрыл в живом реестре и почему. Счётчик по причинам, а
        # не число: «сняли убыточные» и «сняли строки со сметой без ожидания»
        # — разные новости, вторая говорит о дефекте генератора.
        "swept": {
            "closed": len(swept),
            "by_reason": _count_by(
                [{"reason": row.get("dropped_reason")} for row in swept],
                "reason"),
            "unavailable": sweep_error,
        },
...
def main() -> int:
    ...
    # смотрит человек.
    # own_phrases — тот же снимок структуры, что защищает семантику от
    # минусации, и он же отвечает на обратный вопрос «чего мы НЕ купили».
    # Один источник на оба рычага: разъедься они, один и тот же запрос был бы
    # одновременно нашей закупкой (не минусуем) и не нашей (докупаем).
