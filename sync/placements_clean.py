...
from sync.placements import direct, journal, plan as planner
...
def main(argv=None) -> int:
    ...
    _out("режим: %s, кабинетов %d, ворота топ-%d по кликам"
    ...
    for account, logins in ready:
        for login in logins:
            try:
                res = run_login(account, login, apply, args.top_n, use_db)
                for key in totals:
                    totals[key] += res[key]
