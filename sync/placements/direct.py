...
#
# ЕПК (UNIFIED_CAMPAIGN) добавлена на вырост: живой такой кампании ни в одном
# из кабинетов пока нет (замер 08.09.2026: Russever 0, 19 из 20 клиентов EDU
# 0), но Директ переводит на этот формат все новые кампании, и молчаливый
# отказ по типу означал бы, что свежая РК не чистится и человек об этом не
# узнает. ExcludedSites у неё документирован как общее поле кампании; если
# Директ всё же откажет, отказ придёт на одну кампанию и будет виден строкой
# «✗», а не потерей такта.
...
# вычищена тогда же. Архивную и сконвертированную Директ обновлять не даст
# вовсе, завершённая по дате больше не покажется.
CLEANABLE_STATES = frozenset({"ON"})
