...
Ф2.2: поверх логистики — изотоническая калибровка (in-sample train, как каскад
Ф1b) → p_pay становится настоящей вероятностью для прогноза выручки; AP/гейт
считаются на СЫРЫХ p (изотоника монотонна → ранжирование тождественно).
...
def _train_one_point(rows, point: str, today: date) -> dict[str, Any]:
    ...
    p_cal = calibrator.predict(p)   # только для наблюдаемости (avg_p_cal); AP/гейт — на сырых p
    run = {
        ...
        "stage_metrics": {"ap_model": ap_model, "ap_pilot": ap_pilot,
                          ...
                          "avg_p_raw": float(p.mean()), "avg_p_cal": float(p_cal.mean()),
