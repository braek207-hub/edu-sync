...
def combine(observations: List[Dict[str, float]]) -> Optional[Dict[str, float]]:
    """Обратновзвешенный по дисперсии свод наблюдений {eps, rel_error}.

    Веса 1/σ² — точное наблюдение весит больше; ошибка свода √(1/Σw) меньше
    любой одиночной. Стандартная сводка независимых оценок одной величины.
    """
    ...
    return {
        "eps": sum(v * w for v, w in weighted) / total_w,
        "rel_error": math.sqrt(1.0 / total_w),
        "n": len(weighted),
    }
