...
SPEC: Dict[str, Dict[str, Any]] = {
    ...
    "consolidate_min_expected_payments": {
        # Сколько ожидаемых оплат новая кампания должна набрать за горизонт,
        # чтобы вынос считался проверяемым. Общий power.MIN_EXPECTED_PAYMENTS
        # не трогается — это порог вердикта по самой новой кампании, и его
        # ослабление здесь означает «вердикт слабее, но кампания собирается».
        "default": 25, "min": 5, "max": 100,
        "about": "вынос связок: минимум ожидаемых оплат новой кампании за "
                 "горизонт (меньше — вердикт грубее)",
    },
