...
SPEC: Dict[str, Dict[str, Any]] = {
    ...
    "approval_lanes": {
        # Полосы, чьи действия едут в кабинет только после «да» человека в
        # Telegram (sync/agent/approval.py). НЕ тень: действие проходит все
        # гейты и рельсы риска, ложится в журнал pending_approval и ждёт
        # слова. Пусто — дефолт кода (suspend + allocation, решение Павла
        # 03.09.2026: паузы и деньги — с апрувом, пока контур не заслужил
        # доверие).
        "default": None, "nullable": True, "kind": KIND_LANE_LIST,
        "about": "полосы под апрувом человека в Telegram; пусто — "
                 "suspend+allocation",
    },
    "placement_blocklist": {
