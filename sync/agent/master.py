...
**Адрес кампании.** Кабинет у Мастера взять негде: `campaigns.get` о нём
молчит, значит и справочник «кампания → логин» (он строится тем же методом)
его не содержит. Кабинет выводится из проекта кампании — того же поля фактов,
по которому агент раскладывает расход, — и только если проект однозначно
сводится к одному логину на видимых кампаниях. Неоднозначный проект — отказ с
причиной, а не догадка: идея с чужим кабинетом в адресе уедет в чужую очередь
и посчитается по чужому порогу.
"""
