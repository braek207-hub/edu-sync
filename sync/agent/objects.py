...
def expansion_candidates(
    ...
    *,
    ...
    ни с одной ключевой фразой кабинета, в том числе чужой кампании: там он
    уже управляем.

    **Что куплено — из снимка структуры кабинета** (own_semantics поверх
    agent_db.load_keywords_by_campaign), а не из колонки matched_key отчёта
    запросов. Тот же источник и та же ошибка, что у минусации: в matched_key
    Директ возвращает и подбор автотаргетинга, и ключами кабинета оказались
    1,9 % совпадающих с запросом строк. Здесь ошибка работала в обратную
    сторону — не «нельзя минусовать мусор», а «нечего расширять»: замер
    29.08.2026 на окне 30 дней снимал 27 520 запросов из 31 745 как «уже
    купленные», и до отбора доживали 35 запросов с двумя конверсиями вместо
    281, 31 тыс ₽ расхода вместо 458 тыс. Генератор запусков молчал не от
    строгости порогов, а потому что вход был вычищен чужой колонкой.

    bought — обязательный аргумент, не умолчание: значение по умолчанию
    вернуло бы прежний источник ровно там, где вызывающий о нём забыл, и
    рычаг снова замолчал бы молча. Сравнение по НАБОРУ СЛОВ, как у минусации
    (_phrase_words): «куплен» значит «есть ключ ровно из этих слов», а
    широкий ключ, приведший запрос, покупкой запроса не является — на этом
    генератор и стоит.
    ...
    if cpa_limit <= 0:
        return []
    # Кабинетом, а не кампанией: запрос, купленный ключом соседней кампании,
    # уже управляем — заводить его второй раз значит столкнуть свои же
    # кампании на одном аукционе.
    bought_phrases: set = set()
    for keys in (bought or {}).values():
        bought_phrases |= set(keys)
    totals: Dict[str, Dict[str, Any]] = {}
    for q in queries:
        phrase = str(q.get("query") or "").strip().lower()
        if not phrase or _phrase_words(phrase) in bought_phrases:
            continue
        slot = totals.setdefault(phrase, {
            "query": phrase, "cost": 0.0, "clicks": 0, "conversions": 0,
            "campaigns": set(),
