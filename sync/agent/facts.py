...
Рядом с суммой лежит scored_leads — сколько лидов дня скор вообще получили. Скоринг
идёт по поведению визита и требует client_id, проставленного не на всех лендах, поэтому
знаменателем среднего скора служит он, а не eff_leads: деление на eff_leads мерило бы
покрытие скорингом, а не качество когорты (sync/agent/quality.py).
...
def assemble_facts(
    ...
    for r in lead_rows:
        ...
        if r["lead_id"] in p_pay_by_lead:
            # Знаменатель среднего скора (quality.py). Проверка на наличие, а
            # не на ненулевое значение: p_pay = 0 — это замер, а не пропуск, и
            # выкидывание таких лидов красило бы качество тем сильнее, чем
            # хуже трафик.
            slot["scored_leads"] += 1
