...
def portfolio_targets(
    ...
    for login, campaigns in sorted(by_login.items()):
        ...
        #
        # Отложенным ростом признаётся только та часть невязки, что приходится
        # на САМУ ПРИБАВКУ, и бюджет не опускается ниже факта расхода. Ниже
        # факта невязка означает другое: солвер сам решил сократить кабинет —
        # кампании с β ≥ 1 шагают на BETA_SUPERLINEAR_STEP независимо от λ, и
        # у кабинета из одних таких сумма целей не уравнивается с бюджетом
        # никаким порогом.
        #
        # Живой случай 28.08.2026, кабинет account4 (обе кампании β ≥ 1): факт
        # 698 112,80 ₽, Σ целей 607 054,61 ₽ — ровно /1.15. Роста не было
        # вовсе (room = 0, capped_by="room"), а отчёт печатал
        # growth_rub = −91 058,19 ₽ и deferred_growth_rub = +91 058,19 ₽, то
        # есть отрицательный «рост» и отложенную прибавку, которой никто не
        # давал; экран «Деньги» Panda-BI берёт deferred прямо отсюда и
        # показывал эти рубли владельцу. Заодно подмена бюджета на Σ целей
        # обнуляла sum_residual — единственную прямую проверку «сумма на
        # уровне кабинета» — ровно там, где сумма не сошлась.
        deferred = 0.0
        if target_sum < budget - GROWTH_RESIDUAL_RUB:
            deferred = round(min(budget - target_sum, budget - fact_cost), 2)
            budget = max(target_sum, fact_cost)
        ...
        accounts[login] = {
            ...
            # взялась разница. Прибавка неотрицательна по построению: бюджет
            # выходит из account_budget не ниже факта (там max) и не опускается
            # ниже него при снятии невязки выше. Отрицательный «рост» — не
            # сокращение, а сломанный инвариант; сокращение кабинета читается
            # парой target_sum_28d < cost_28d и невязкой sum_residual.
