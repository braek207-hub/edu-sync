...
сравнения, срок и машинно проверяемый критерий.

**Смета проверки пуста, и это не пропуск.** Ценность выигранного теста —
информация, а не рубли: посчитать её заранее нечем. А раз ожидания нет, то и
цены проверки идея нести не вправе — считаем обе величины или ни одной
(ideas/limits.unpaired_reason). Расход кампании за срок замера при этом никуда
не делся, он лежит в detail: человеку перед запуском теста надо знать, сколько
денег проходит через окно, но очередь реестра делит ожидание на СМЕТУ, а сметы
у предложения нет — риск-бюджетом оно не платит (writer/lanes.RISK_PAYING_LANES).
...
REASON_NO_COST = (
    "у кампании нет расхода: тест нечему улучшать, и объём под ударом не от "
    "чего отмерить")
REASON_THIN_POWER = (
...
def _one(row: Dict[str, Any], test_kind: str, entry: Dict[str, Any],
    ...
    return {
        ...
        # Адрес теста — кампания и тип, и только они. Срок и числа плавают от
        ...
        "expected_rub": None,
        # Сметы нет, и это следствие пустого ожидания: считаем обе величины
        # или ни одной (ideas/limits.unpaired_reason). Здесь стоял расход
        # кампании за срок замера — «под ударом весь объём», — и сто тринадцать
        # таких строк давали экрану предложений 108 946 589 ₽ заявленной цены
        # проверки при нулевой заявленной выгоде (замер 29.08.2026). Число при
        # этом не было сметой в смысле реестра: смета — списание риск-бюджета
        # полосы (writer/risk.action_risk), а предложение риск-бюджетом не
        # платит вовсе. Сам объём под ударом не потерян — он ниже, в detail,
        # где его читает человек, а не делит очередь.
        "test_cost_rub": None,
        ...
        "detail": {
            "change": change,
            # Сколько денег кампании проходит через срок замера. Проигравший
            # тест портит кампанию целиком на всё это время, и человеку перед
            # запуском теста это знать надо — но это оборот, а не цена
            # проверки, и в колонку сметы он не идёт (см. test_cost_rub).
            "campaign_cost_at_risk_rub": round(facts["daily_cost"] * horizon, 2),
