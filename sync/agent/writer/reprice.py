...
Что считается уже переоценённым. Не «колонка risk_basis заполнена», а
«основание совпало с сегодняшним»: основание перечисляет множители модели
словами и потому служит её отпечатком. Правило про непустую колонку работало
ровно один раз — на переходе от «весь объект» к дельта-модели; на следующей же
правке цены (29.08.2026, ошибка раскладки) оно законсервировало бы весь журнал
в старых ценах.
...
REASON_ALREADY_DELTA = "цена уже посчитана сегодняшней моделью (основание совпало)"
...
def plan(
    ...
    Свежей считается строка, чьё ОСНОВАНИЕ совпало с тем, что написала бы
    сегодняшняя модель, — а не просто непустое. Разница измерена: 29.08.2026
    цена переноса перестала списывать всю переложенную сумму и стала считать
    ошибку раскладки (exposure.bid_modifier_exposure), и по прежнему правилу
    «risk_basis заполнен — не трогаем» весь журнал прошлой дельта-модели
    остался бы в старых ценах ровно с тем же следствием, ради которого этот
    модуль и написан: неделю после починки агент почти не может действовать.

    Основание — отпечаток модели: оно перечисляет её множители словами, и
    любое изменение формулы меняет строку. Поэтому сравнение с ним переживает
    и следующую правку цены, чего флаг «колонка заполнена» не умеет по
    построению.
    """
    ...
    for row in rows:
        ...
        if _percent(row) is None:
            ...
            continue
        stored = str(row.get("risk_basis") or "").strip()
        if stored and stored == basis_for(row, share, daily_cost_by_campaign):
