...
def action_risk(
    ...
    Запуск считается ДО потолка объекта и мимо справочника расходов, потому
    что объекта ещё нет: у новой кампании нет ни своего расхода, ни истории,
    и медиана чужих кампаний к ней отношения не имеет. Под ударом ровно тот
    недельный лимит, который ей открывают, и ровно тот срок, который назначен
    её наряду (exposure.launch_exposure). Пусти запуск общим путём — при
    пустом справочнике он стоил бы +inf («оценить не от чего») там, где цена
    известна точнее, чем у любого другого рычага.
    """
    launch = exposure_mod.launch_of(action.get("exposure"))
    if launch is not None:
        return round(launch["daily_rub"] * launch["days"], 2)
...
def object_cap(
    ...
    У запуска потолок объекта совпадает с ценой самого действия: «весь расход
    кампании за горизонт» и есть тот лимит, который ей открывают, за тот же
    горизонт. Считай его по справочнику — потолком новой кампании стала бы
    медиана чужих, и полоса списала бы за запуск цену случайной соседки.
    """
    launch = exposure_mod.launch_of(action.get("exposure"))
    if launch is not None:
        return round(launch["daily_rub"] * launch["days"], 2)
