...
# На сколько дней сверка сумм отстаёт от последнего дня витрины. Сутки — это
# срок, за который день ОСЕДАЕТ у самого Директа: витрина собирает день сразу
# после полуночи, а источник продолжает дописывать его в течение следующих
# суток, и последний день витрины систематически короче источника.
#
# Замер 28.08.2026 по 21 дню: девятнадцать закрытых дней сходятся с витриной
# ДО РУБЛЯ (расхождение 0.0 у каждого), и всё расхождение окна — один только
# последний день витрины: 27.08 источник 1 090 223 ₽ против витрины
# 535 149 ₽ (49 % дня ещё не осело). В сумме по окну это 7,76 % при допуске
# 1 % — то есть красный гейт КАЖДЫЙ прогон, откат заморожен навсегда, и
# заморожен не порчей данных, а расписанием сборки.
#
# Отключением проверки это не является, как и у аномалии объёма рядом:
# порченый день по-прежнему краснеет, вердикт приходит на сутки позже — ровно
# тогда, когда о дне вообще есть что сравнивать.
SUM_SETTLE_DAYS = 1
...
def with_source_checks(gate: Dict[str, Any], include_volume: bool,
    ...
    у битой витрины в порядке. Окно сверки кончается не на сегодня и даже не
    на последнем ШИРОКОМ дне витрины, а на последнем ОСЕВШЕМ дне
    (SUM_SETTLE_DAYS): день, который витрина собрала сразу после полуночи, а
    источник дописывал ещё сутки, короче источника у КАЖДОГО прогона, и
    сверка с ним краснела бы всегда. Замер и числа — там же, у константы.
    ...
    settled_to = (date.fromisoformat(latest)
                  - timedelta(days=SUM_SETTLE_DAYS)).isoformat()
    settled = [(day, cost) for day, cost in daily if day <= settled_to]
    if settled:
        checks.append(check_sum_reconciliation(
            sum(cost for _, cost in settled),
            agent_db.mart_cost_total(date_from, settled_to)))
    if not checks:
        return gate
