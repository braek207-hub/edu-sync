...
## Площадка платного трафика: движок безопасен на узкой выборке ✅ ЗОНД P5 (2026-07-19)

Скрипт: `scripts/probe_gcc_ad_engine.py`. Замер 2026-06-19…07-18.

Обрезка Метрики бьёт по **хвосту**, поэтому на выборке «только реклама»
(`filters=ym:s:lastsignTrafficSource=='ad'`) она почти не работает:

| Набор измерений (фильтр: реклама) | Визиты | Потеря |
|---|---|---|
| дата + источник (эталон) | 90 340 | — |
| + домен | 90 340 | 0.00% |
| + домен + **движок** | 89 942 | −0.44% |
| + домен + движок + кампания | 87 509 | −3.13% |
| + домен + движок + utm_source + кампания | 87 420 | −3.23% |

Сравнить с −42% у Бахрейна на ОБЩЕМ запросе с движком (зонд 2026-07-18) — измерение
не «сломано», просто его нельзя скрещивать с длинным хвостом мелких комбинаций.

- **Площадка известна у 100% платных визитов**: Google Ads 49.3%, Instagram 36.9%,
  Facebook 13.8%. Корзины «без движка» нет вовсе.
- Отсюда прод-схема: реклама — своим запросом с движком и кампанией, разница до
  запроса без кампании пишется строкой-остатком с площадкой (`ad_engine_residual`),
  нерекламный трафик — прежним набором с фильтром `!='ad'` (иначе задвоение).
- Результат на живых данных: платный тотал 90 340 = эталон (потерь нет), доля визитов
  без площадки 23% → **0.4%**. Google.Adwords 26 804 → 44 357 визитов: большая часть
  безметочной рекламы оказалась Google, его CPC был завышен почти вдвое.
