...
человеком). Всё, что здесь проверяется, — про цену их перепутать.
  ...
  * ЧУЖОЙ КАБИНЕТ В АДРЕСЕ. campaigns.get о Мастере молчит, но отчёт запросов
    Reports API — нет, и справочник «кампания → логин» собирается из обоих:
    раз отчёт КАБИНЕТА вернул кампанию, она этого кабинета. Не нашлось и там
    — кабинет выводится из проекта, и проект, живущий в двух кабинетах,
    обязан дать отказ, а не догадку: идея с чужим кабинетом посчитается по
    чужому порогу;
...
def test_the_directory_answers_before_the_project_guess():
    """Кампания есть в справочнике — проект не спрашивают вовсе.

    Справочник собирается из ответов самих кабинетов, в том числе из отчёта
    поисковых запросов, который Мастера отдаёт наравне со всеми. Замер
    29.08.2026: пять кампаний Мастера на 1 678 168 ₽ отказывались адресоваться
    по проекту («встречается сразу в нескольких кабинетах») — при том, что
    отчёт кабинета вернул по каждой тысячи строк запросов.
    """
    # Проект неоднозначен: тот же «vse» живёт и в чужом кабинете. Прежде это
    # был отказ; теперь справочник отвечает раньше, чем до проекта доходит.
    facts = _facts() + [_fact("300", cost=100_000.0, eff_leads=20.0)]
    login_by_campaign = dict(_login_by_campaign(),
                             **{"300": OTHER, "900": ACCOUNT})
    ...
    assert view["rows"][0]["account"] == ACCOUNT
