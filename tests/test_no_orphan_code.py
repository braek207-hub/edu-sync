...
ALLOWED: Dict[str, str] = {
    ...
    # Рычаг смены цели оптимизации (задача 21). Генератора у него нет и быть
    # пока не может: расчётный такт не считает строк о целях вовсе — ни
    # достижений цели за окно, ни конверсии кампании по чужой цели, а
    # выдумывать их рычаг не вправе (ровно это он и отказывается делать). Ждёт
    # своего источника; если к бете запись всё ещё здесь — цель в бою не
    # меняется никем, кроме человека.
    "diff_goal": "нужен источник достижений целей: генератора Э3 пока нет",
    # Рычаг смены стратегии (задача 22) — тот же случай: расчётный такт не
    # считает ни достижений цели, ни конверсии кампании под ЧУЖОЙ стратегией.
    "diff_strategy": "нужен источник достижений целей: генератора Э3 пока нет",
    # Рычаг аудиторий и ретаргетинга (задача 23). Входа у него сегодня нет с
    # ДВУХ сторон, и обе названы замером: расчётный такт не снимает среза по
    # условиям ретаргетинга (план беты, задача 16а), а читатель состояния
    # кабинета (agent_e1._actual_modifiers) не запрашивает
    # RetargetingAdjustmentFieldNames — то есть уже стоящих корректировок
    # прогон не видит, и рычаг честно отказывает вместо догадки. Плюс замер
    # 26.08.2026: в четырёх кабинетах EDU привязок условий к группам ноль,
    # корректировать физически нечего. Если к бете запись всё ещё здесь —
    # ретаргетинга у EDU так и не появилось.
    "diff_audience": "нет среза по условиям ретаргетинга и чтения их корректировок",
    # Рычаг географии (задача 24). Не хватает ДВУХ источников сразу, и ни один
    # рычаг выдумать не вправе: расчётный такт не считает расхода и конверсий
    # по регионам (срез региона в витрине есть, но приходит НАЗВАНИЕМ, а API
    # записи требует числовой RegionId — sync/agent/segments.SEGMENT_FIELDS), и
    # не строит карту покрытия кабинета, без которой не видно каннибализации.
    # Если к бете запись всё ещё здесь — географию в бою не меняет никто,
    # кроме человека.
    "diff_geo": "нужны расход по числовым RegionId и карта покрытия кабинета",
}
