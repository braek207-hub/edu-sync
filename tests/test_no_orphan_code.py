...
ALLOWED: Dict[str, str] = {
    ...
    # Реестр идей (задача 10). Потребителей у него по построению ещё нет:
    # секцию отчёта и питание такта записи делает задача 11, генераторы,
    # которые пишут в реестр, — задачи 12–15. Если к бете записи всё ещё
    # здесь, реестр в бою не подключён: идеи по-прежнему живут внутри такта.
    # Гейт видит имя как «вызванное», если оно встретилось хоть где-то в
    # боевом коде, — а `rank` там есть локальной переменной (sync/ml_score.py,
    # sync/crm.py). То есть на этом имени гейт молчал бы и без записи;
    # запись стоит здесь именно поэтому: пусть отсутствие потребителя будет
    # видно человеку, раз механика его не видит.
    "idea_id": "потребители — отчёт (задача 11) и генераторы идей (задачи 12–15)",
    "subject_key": "потребители — отчёт (задача 11) и генераторы идей (задачи 12–15)",
    "rank": "потребители — отчёт (задача 11) и генераторы идей (задачи 12–15)",
    "upsert": "потребители — генераторы идей (задачи 12–15)",
    "load": "потребители — отчёт и план записи (задача 11)",
    "mark": "потребитель — такт записи: queued/running/done (задача 11)",
