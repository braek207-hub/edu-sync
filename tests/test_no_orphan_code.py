...
ALLOWED: Dict[str, str] = {
    ...
    # Полосы (задача 4): lane_of потребителя УЖЕ получил — writer/expectation
    # берёт по нему срок замера, — и запись отсюда снята. policy_of ждёт
    # отбора, который применяет лимиты полос (lanes.select, задача 7); если к
    # бете запись всё ещё здесь, полосы посчитаны и выброшены.
    "policy_of": "потребитель — lanes.select, задача 7 плана беты",
    # Классы достоверности (задача 5): tier_of зовёт тот же отбор — класс
    # решает, платит ли действие риском полосы и доходит ли до кабинета
    # вообще. Если к бете запись всё ещё здесь, классы посчитаны и выброшены:
    # арифметика снова стоит в очереди, а предложения снова применяются.
    "tier_of": "потребитель — lanes.select, задача 7 плана беты",
    ...
    # Цена такта с взаимозачётом переносов (задача 8): её зовёт тот же отбор,
    # что и полосы с классами, — там набор действий такта известен целиком, а
    # поштучный action_risk в agent_e1 знает только одно действие и зачесть
    # встречный перенос не может. Если к бете запись всё ещё здесь, полоса
    # перераспределения снова платит за перенос обеими сторонами.
    "net_risk": "потребитель — отбор набора такта (lanes.select), задача 7 плана беты",
}
