...
def _bundle(events=40, romi=2.6, **over):
    ...
    bundle = {
        ...
        "cpa_rub": 900.0,
        # Ценность лида выбрана так, чтобы идея ОКУПАЛА свою проверку
        # (ideas/limits.unprofitable_reason): 23 340 ₽ ожидания против 14 000 ₽
        # цены замера. При 2 400 ₽ связка проходила все пороги генератора и
        # снималась реестром как убыточная — то есть помощник отдавал бы
        # связку, которая идеей стать не должна.
        "value_per_lead_rub": 6_000.0,
