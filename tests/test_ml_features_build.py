...
def test_assemble_pure(monkeypatch):
    ...
    sessions = {"c1": [{"visit_ts": datetime(2024, 12, 31, 10, 0, 0), "visit_duration": 200.0,
                        "bounce": 0, "page_views": 3.0, "device_category": "desktop", "source_engine": "ad"}]}
    rows, maturation = b.assemble(leads, sessions, deadlines=[date(2025, 8, 20)], today=date(2026, 7, 23))
...
def test_registry_feature_keys_present_in_build_feature_rows():
    ...
    rows = build_feature_rows([lead], sessions={}, deadlines=[date(2025, 8, 20)],
                              today=date(2026, 7, 23))
