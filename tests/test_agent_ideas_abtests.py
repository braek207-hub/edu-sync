...
def test_unreleased_kind_is_skipped_with_a_named_reason():
    ...
    # Рычаги задач 22–24 ещё не написаны — их типы отбраковываются с
    # названной причиной, а не молча. Смены цели здесь больше нет: рычаг
    # написан (задача 21), и её место — среди кандидатов ниже.
    assert {"strategy", "audience", "geo"} <= kinds
    assert all(row["reason"] for row in skipped)


def test_the_goal_test_became_a_candidate_with_its_lever():
    # Каталог обещал тест смены цели с самого начала, но до задачи 21 обещать
    # его было нечем. Появился рычаг — тип обязан выйти из отбраковки в
    # кандидаты; иначе каталог остался бы списком намерений.
    assert "goal" in _kinds()
